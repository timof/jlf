...
// - sql_prune_*(): deletes by garbage collection (should only be called with admin privileges). Will return number of deleted entries. now moved to garbage.php
...
if( ! function_exists( 'sql_people' ) ) {
  function sql_people( $filters = array(), $opts = array() ) {
    ...
    $opts['filters'] = sql_canonicalize_filters( 'people', $opts['filters'], $opts['joins'], $opts['selects'], array( // hints
...
function sql_sessions( $filters = array(), $opts = array() ) {
  global $now_unix, $session_lifetime_seconds;
  $joins = array(
