...
function form_row_posten( $s ) {
  ...
  open_td( $problem );
    ...
    close_div();
  open_td( "bottom $problem" );
    if( $hauptkonten_id ) {
      filter_unterkonto( $s, array( 'hauptkonten_id' => $hauptkonten_id ), '' );
    } else {
      hidden_input( $s.'unterkonten_id', '0' );
    }
  open_td( 'bottom', '', string_view( $beleg, $s.'_beleg' ) );
  open_td( "bottom $problem_summe", '', price_view( $betrag, $s.'_betrag' ) );
}
...
open_form( '', 'action=update' );
  ...
  open_fieldset( 'small_form', '', 'Buchung ' . ( $buchungen_id ? "$buchungen_id" : '(neu)' ) );
    ...
    open_table( 'form' );
      open_tr( 'smallskips' );
        open_th( 'top' );
          open_div( 'tight', '', 'Kontoart' );
          open_div( 'tight', '', 'Geschaeftsbereich' );
        open_th( 'top' );
          open_div( 'tight', '', 'Seite' );
      ...
      open_tr( 'smallskips' );
        open_td( 'right', "colspan='8'", html_submission_button( ' Neuer Posten ', true, false, 'addS' ) );
      open_tr( 'medskip' );
      ...
      open_tr( 'smallskips' );
        open_td( 'right', "colspan='8'", html_submission_button( ' Neuer Posten ', true, false, 'addH' ) );
      open_tr( 'smallskips' );
        open_td( 'right medskip', "colspan='8'", html_submission_button( 'Speichern', 'bigbutton', '', 'save' ) );
    close_table();
  close_fieldset();
close_form();
